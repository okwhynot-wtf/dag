...
\vspace{0.45em}
\section*{The motion is forced too}

The main story fixes the alphabet by elimination. The corpus now
fixes the motion the same way, and this stretch belongs to the
audited main line. Ask what a second-order update rule on a row of
two-letter sites can look like if no two histories are ever allowed
to merge, and the answer is forced: the rule must flip its past layer
(the xor form), and the remaining freedom, the coupling between
neighbours, survives in exactly two mirror versions of one rule once
three fairness conditions are imposed (neither neighbour preferred,
neither letter preferred, and something actually distinguished). The
survivor is a wave equation living natively on the alphabet. Its step
is two bounces by definition, running it backwards is swapping the
bounces, and its spectrum is set by its boundary: the same pulse
under the same rule repeats every 4 ticks on a ring of four sites and
every 10 ticks between fixed ends. Every finite rhythm is exactly
two flips of the fundamental beat, and for plain rotations the two
flips can be written down in closed form. The circle of fifths closes
after exactly twelve steps, proved as a fact about order with no
claim about sound, and the general law behind it (period equals size
over greatest common divisor) is a theorem. Any state, under any
coupling, must return to itself within the capacity of its carrier:
a bounded return, forced by losslessness alone. The two mirror
versions of the rule differ by a flip, and that flip has been placed
by proof. On rows whose size is a multiple of four the two versions
are one dynamics wearing two labels: an explicit relabelling of
states carries one into the other. On every other size no
relabelling exists, however clever, and the two versions can be told
apart by looking: one holds the silent row perfectly still, and the
other holds nothing still. At every size the flipped version is the
plain version dressed by a four-beat schedule of label flips, so the
difference always has the flavour of packaging, and only on the
sizes divisible by four can the schedule be traded for a single
once-and-for-all relabelling.

\vspace{0.45em}
\section*{An experimental annex}

Alongside the main development, the repository keeps a small
quarantined annex of machine-checked probes; nothing in the main
story depends on them. At the edge of the ladder sits the space of
complete histories, which no list can exhaust, in which every rung
embeds, and where forgetting becomes possible; inside the arena
forgetting is excluded at every stage, so the edge is the one place
the system provably loses, and that loss is itself a theorem. The
annex also keeps the proofs that place the mirror flip of the wave
rule, the concrete exhibits behind the wave story, and the period
table for larger carriers, where the observed pattern is recorded
honestly as conjecture. The annex records what it cannot do
as carefully as what it can: no continuum, no real frequencies, and
the closed-form wave-period law is left, honestly, as a wall.
