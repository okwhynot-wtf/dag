...
/-- T-2 classified remnant: Fund exempt + swap necessity + UF append glue.
    Target rename: `milestone_T2_tick_id` when `tick_identification` lands. -/
theorem milestone_T2_tick_id_classified :
    ...
    (∀ {S : Type} {u : S → S} {A : Bridge.Dil.Archive S u}
      ...
      ∃ _i : Bridge.Dil.ArchiveIso A
        (Bridge.Dil.freeOnBase S u (A.E 0) A.z0), True) ∧
    Bridge.Alphabet.Kmin = 2 :=
  ⟨Bridge.TickSimulation.tick_identification_classified.1,
   Bridge.TickSimulation.tick_identification_classified.2.1,
   Bridge.TickSimulation.tick_identification_classified.2.2.1,
   fun fa => (Bridge.TickSimulation.tick_identification_classified.2.2.2.1 fa).1,
   Bridge.Alphabet.Kmin_eq⟩
