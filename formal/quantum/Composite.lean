...
The hard wall of `HILBERT.md` records that the spine has no composition.
This module builds the smallest composite by hand, the four-state
carrier `Bool × Bool` with the two componentwise flips, and classifies
its natural orthogonality space. The findings revise the wall in both
directions at once.

**Against the composition conjecture.** Orthogonality generated by the
two componentwise acts still has rank exactly two
(`rank_cap_two`): the union of two act-graphs on four states is the
four-cycle, and no three states are pairwise orthogonal. Composition
alone does not open rank.

**What composition does buy.** The joint flip `act = a₁ ∘ a₂` is
orthogonally invisible: every state has exactly the orthogonality
neighbourhood of its joint flip (`flip_indiscernible`), so the two-act
orthoset fails irredundancy (`irredundancy_fails`), a third failure
direction beside the spine's rank failure and the discrete reading's
superposition failure. Invisibility of a global involution is the
signature of a *phase*: the pair `{x, act x}` behaves as one ray.
Passing to rays (`SameRay`, the two-element orbits of `act`) the
structure collapses exactly onto the fundamental: the ray class is the
xor of the components (`rayClass`), orthogonality holds precisely
between opposite ray classes (`perp_iff_ray_negation`), the joint flip
descends to the identity on rays (`act_ray_trivial`), and the
quarter-turn `J` of `SpineFacts.sqrt_model_exists` descends to ray
negation (`J_is_ray_negation`). At ray grade the composite is the
classical bit again.

**The returned arrow.** Every point of the quarter-turn is an
asymmetric-step point in the sense of the pointwise classification,
case (iii): `J (J x) ≠ x` everywhere (`J_asymmetric_everywhere`), and
`J` commutes with the joint flip (`J_comm_act`). The shape eliminated
at the base returns on the composite as the square root of the
survivor, and it is orthogonally a ray negation.

**The sharpened residue.** Composition supplies phase invisibility,
the ray quotient, and the complex-structure candidate, and it still
supplies no superposition: the ray-grade structure is the two-point
classical orthoset. Superposition remains the sole quantum clause
after composition, which strengthens the reading of
`Axiom.Orthoframe`: neither one system nor two systems force it.
